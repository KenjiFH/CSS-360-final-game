...
class SpriteObject:
    def __init__(self, game, path='resources/sprites/static_sprites/PNGNAMEHERE.png', pos=(10.5, 3.5), scale=1, shift=0):
        self.game = game
...
class AnimatedSprite(SpriteObject):
    def __init__(self, game, path='resources/sprites/animated_sprites/PNGNAMEHERE.png', pos=(10.5, 3.5), scale=1, shift=0, animation_time=120):
        super().__init__(game, path + '/5.png', pos, scale, shift)
    ...
    def get_images(self, path):
        images = deque()
        for file_name in sorted(os.listdir(path), key=lambda f: int(f.split('.')[0])):
